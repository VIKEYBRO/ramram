...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if ongoing_attack:
            response = "⚠️ An attack is already in progress. Please wait until it is finished before starting a new one."
            bot.reply_to(message, response)
            return
...
def start_attack_reply(message, target, port, time):
    ...
    bot.reply_to(message, f"🔥 Starting BGMI attack on {target}:{port} for {time} seconds...")
